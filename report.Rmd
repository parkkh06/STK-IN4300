...
ggplot2::ggplot(data = data %>% 
                  ...
                  dplyr::summarize(Average_rented_bike = sum(Rented.Bike.Count)),
...
- Rental demand differs substantially in Winter.
...
df_x_cont <- data %>% dplyr::select(where(~!is.factor(.))) %>%
  dplyr::select(-Rented.Bike.Count)
...
model_1 <- caret::train(Rented.Bike.Count ~ Date + Hour + Seasons + Holiday +
...
model_2 <- caret::train(Rented.Bike.Count ~ Hour + Seasons + Holiday +
                        Temperature.C. + Humidity... + Wind.speed..m.s. + Visibility..10m. + Dew.point.temperature.C. +
                        Solar.Radiation..MJ.m2. + Rainfall.mm. + Snowfall..cm., 
                      data = df_mdl_input, method = "lm", trControl = train_control)
...
# correlation
cor(x = cbind(df_x_cont_scaled, df_y), method = "pearson")

plot(cbind(df_x_cont_scaled, df_y))

# feature selection

forw <- lm(Rented.Bike.Count ~ 1, data = df_mdl_input)
back <- lm(Rented.Bike.Count ~ ., data = df_mdl_input)


step(forw, direction = "forward", scope=list(lower=forw, upper=back))


feature_selection <- stepAIC(lm(Rented.Bike.Count ~ Date + Hour + Seasons + Holiday +
                        Temperature.C. + Humidity... + Wind.speed..m.s. + Visibility..10m. + Dew.point.temperature.C. +
                        Solar.Radiation..MJ.m2. + Rainfall.mm. + Snowfall..cm.), direction = "both")


train_set <- df_mdl_input %>% dplyr::sample_frac(0.8)
...
mlr_model <- lm(Rented.Bike.Count ~ Date + Hour + Temperature.C. + Humidity... + Wind.speed..m.s. + Visibility..10m. + Dew.point.temperature.C. + Solar.Radiation..MJ.m2. + Rainfall.mm. + Snowfall..cm. + Seasons + Holiday, data = train_set)
